...
def init_sqlite_db():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT ,username TEXT,surname TEXT, '
...
def new_user():
    if request.method == "POST":
        ...
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT into users (username,surname,email,contact) VALUES (?,?,?,?)",
                            (username, surname, email, contact))
                con.commit()
                msg = username + "was succcessfully assed to the database."
        except Exception as e:
            con.rollback()
